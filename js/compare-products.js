...
const renderCarouselProducts = (data) => {
    ...
    const contentHTML = data.map((item) => {
        ...
        <div class="products__note">${product.overFolow(item.specifications.brand, 30)}</div>
...
const renderCompare = (data, obj) => {
    ...
    info.forEach((item) => {
        ...
        brand += `<td>${item.specifications.brand}</td>`;
        connectionProtocol += `<td>${item.specifications.connectionProtocol}</td>`;
        capacity += `<td>${item.specifications.capacity}</td>`;
        reliability += `<td>${item.specifications.reliability}</td>`;
        designs += `<td>${item.specifications.designs}</td>`;
        readingSpeed += `<td>${item.specifications.readingSpeed}</td>`;
        writeSpeed += `<td>${item.specifications.writeSpeed}</td>`;
